...
Like the previous mathematics tests, the M1 is mandatory for students, and its properties and results deserve attention. This paper analyzes the M1 applied in the 2024 admission process from a psychometric perspective, i.e., fitting and evaluating different models from the Item Response Theory (IRT) framework to evaluate its appropriateness for measuring what is intended to. This study aims to provide evidence about the psychometric qualities of the M1, which may help judge the validity of its scores and whether its results can be linked to how it was designed. In this sense, the research questions were two: what are the characteristics of the most appropriate IRT model for the M1? How comparable are the information and efficiency of the forms used in the application of M1 according to the most appropriate IRT model?
...
Test calibration in IRT involves obtaining the numeric values of item parameters for each of the models defined above [@baker2004item]. When dealing with a single test form, obtaining item parameter estimates is straightforward and can be achieved using many statistical software programs. However, when multiple test forms are considered, decisions must be made about the calibration approach to ensure item parameters across forms are on the same scale. This process is known as linking.

Depending on the data collection approach and sample characteristics, we can choose one of three commonly used IRT linking methods: separate calibration, concurrent calibration, and fixed calibration [@lee2018irt]. Given that students were randomly assigned to each form of the M1 test, this data collection approach aligns with the specifications of a random groups design. Therefore, we choose the concurrent calibration approach to obtain item parameter estimates across forms, allowing us to obtain parameters for both common items and form-specific items.

In more detail, the concurrent calibration process combines all item response data across forms into a single dataset for parameter estimation in one computer run. Items that are unique to a particular form are treated as missing in the other forms. For this calibration approach, the marginal maximum likelihood estimation with the expectation-maximization algorithm was employed, as implemented in the mirt R package [@chalmersmirt], which was used for all model estimations.   

## Model fit
To assess the fit of the IRT models in this study to the observed response data, we examined both absolute and relative model fit. Absolute model fit evaluates how well a model's predicted responses match the actual observed responses. In contrast, relative model fit involves comparing multiple potential models to determine which one provides the most efficient and accurate representation of the dataset. While absolute fit focuses on the alignment between predicted and observed responses for a single model, relative fit aims to identify the most suitable model from a group of well-fitting models.

We evaluated absolute model fit using the M2 statistic [@maydeu2005limited], a limited information model fit test that computes the first and second order marginal residuals of the IRT models. Under the null hypothesis that the model is exactly true in the population, this statistic is asymptotically chi-square distributed. However, under the alternative hypothesis (i.e., lack of fit), it has a non-central chi-square distribution when N is large [@cai2023incremental].
For this test, we conclude that the model adequately fits the data when we fail to reject the null hypothesis (i.e., when the p-value is greater than 0.05). 

We evaluated relative model fit using likelihood ratio (LR) test. This is a statistical significance test that is appropriate for comparing nested models as the case is for the 1PL, 2PL and 3PL models estimated in this study. After confirming that the 3PL model demonstrates adequate absolute fit, we can employ the LR test to evaluate whether simplifying to a 2PL model is justified. If the 2PL model proves to be suitable, we can then conduct an additional LR test to assess if further simplification to a 1PL model is statistically appropriate. This stepwise approach allows us to systematically compare nested models and determine the most parsimonious yet effective representation of the data.

The LR test compares nested models by examining the difference between their maximized log-likelihoods. Specifically, it subtracts the maximized log-likelihood of the more complex model (with more parameters) from that of the simpler model (with fewer parameters) and multiplies the result by *$-2$*. Generally, models with more parameters fit the data at least as well as those with fewer parameters. However, the LR test determines whether removing certain parameters significantly reduces model fit. This approach helps identify the most parsimonious model that adequately represents the data without unnecessary complexity.
...
Item and test information, along with standard errors, are additional metrics for evaluating the performance of an item or test in meeting assessment goals. The item/test information represents the precision with which ability is estimated across the ability continuum. The more information we have in estimating an ability, the less error we have in that ability estimate. Computationally, the standard error of the ability estimate is the reciprocal of the square root of the information [@baker2017basics].

Given that information is highest only at specific point on the ability continuum, we examined test information across the IRT models to ensure that the selected model is peaked around the target ability level for the M1 test. Typically, the point where information is peaked will coincide with the point where the standard error is lowest. 
