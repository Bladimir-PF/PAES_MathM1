...
## Model fit
In order to examine how well the IRT models in this study fit the observed response data, we examined both absolute model fit and relative model fit. Absolute model fit evaluates how well a model's predicted responses match the actual observed responses. In contrast, relative model fit involves comparing multiple potential models to determine which one provides the most efficient and accurate representation of the dataset. While absolute fit focuses on the alignment between predicted and observed responses for a single model, relative fit aims to identify the most suitable model from a group of well fitting models. We evaluated absolute model fit with M2 statistic [@maydeu2005limited]. This statistic is a limited information model fit test that computes the first and second order marginal residuals of the IRT models. Under the null hypothesis that the model is exactly true in the population, this statistic is asymptotically chi-square distributed, however, under the alternative hypothesis i.e lack of fit, it has a non-central chi-square distribution when N is large [@cai2023incremental].For this test, we conclude that the model adequately fits the data when we fail to reject the null hypothesis. i.e p-value is greater than 0.05.

The M2 indicates an adequately fitting model when the p-value is greater than 
.05. 
