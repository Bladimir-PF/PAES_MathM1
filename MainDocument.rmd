...
Relative model fit was assessed using likelihood ratio tests (−2ΔLL). This is a statistical significance test that is appropriate for comparing nested models, as the case is for the 1PL, 2PL and 3PL models. The −2ΔLL test consists of subtracting the maximized log-likelihood of the more complex model from that of the simpler model, multiplying such difference by -2 and comparing it to a $χ^2$ distribution with degrees of freedom equal to the difference in parameters between models to determine its significance. For instance, after checking that the 3PL and 2PL models demonstrate adequate absolute fit, we can employ the −2ΔLL test to evaluate whether constraining some parameters in the 2PL model fits the data better than estimating additional parameters in the 3PL model.
